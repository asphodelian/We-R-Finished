...
# for each state
allAdjustMed <- aggregate(salcost$ADJUSTED_WAGE, by = list(salcost$Work_State), FUN = median)
names(allAdjustMed) <- c("State", "Median Adjusted Wage")
# sorting
allAdjustMed <- allAdjustMed[order(-allAdjustMed$'Median Adjusted Wage'),]

#paid for each state
allPaidMed <- aggregate(salcost$PAID_WAGE_PER_YEAR, by = list(salcost$Work_State), FUN = median)
